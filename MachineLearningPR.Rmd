...
We then set up the data for it to be able to be predicted. Firstly, the data will be run using a decision tree. Followed by a randomForest analysis:
```{r, include=TRUE, echo = TRUE}
...
```
Above, we looked at the data using a Decision Tree to predit the values for A-E. One figure to note is the low accuracy rate hovering around 65%. Now, we look at a different way to conduct the prediction that involves the use of a random Forest tree
